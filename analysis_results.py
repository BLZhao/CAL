...
with open('attention_results/model_CausalGAT_woEdgeAtt/attention_results.pkl', 'rb') as f:  # 'rb'表示以二进制读取模式打开文件
    load_data = pickle.load(f)

with open('attention_results/model_CausalGAT_woEdgeAtt/test_results.pkl', 'rb') as f:
...
# # 创建一个大图来包含所有组的子图
# total_groups = len(attention_data_all)
# fig = plt.figure(figsize=(15, total_groups * 8))  # 调整整体图像大小
#
# # 遍历每个组
# for group_idx, data in enumerate(attention_data_all):
#     n_vars = data.shape[0]  # 变量个数
...
#
#     # 为每个组创建一个子图区域
#     gs = fig.add_gridspec(total_groups, 1)[group_idx]  # 在大图中划分网格
#     subfig = fig.add_subfigure(gs)
#
#     # 在子图区域中创建n_vars+1个子图
#     axs = subfig.subplots(n_vars + 1, 1)
#     subfig.suptitle(
#         f'Segment {attention_group_all[group_idx]} start{split_segments[attention_group_all[group_idx]][0]} end {split_segments[attention_group_all[group_idx]][-1]}')
#
#     # 如果只有一个变量，确保axs是数组
#     if n_vars == 1:
#         axs = [axs]
...
#     # 添加标签时间序列
#     time = np.arange(time_points)
#     axs[n_vars].plot(time, label_all[group_idx], color='red')
#     axs[n_vars].set_title('Labels')
#     axs[n_vars].grid(True)
#     axs[n_vars].set_xlabel('Time')
#     axs[n_vars].set_ylabel('Label')
#
...
#     ind = indices_1[i]
#     # print(np.where(reshaped_attention[ind, :] > 0.95)[0])
#     print(indices_max5[ind])
#     # print(np.argmax(reshaped_attention[ind, :]), max(reshaped_attention[ind, :]))



# graph_idx = 0  # 想要查找的图的索引
# node_mask = (data.batch == graph_idx)
# node_indices = torch.where(node_mask)[0]
#
# edge_mask = (data.edge_index[0].view(-1) >= node_indices.min()) & \
#            (data.edge_index[0].view(-1) <= node_indices.max()) & \
#            (data.edge_index[1].view(-1) >= node_indices.min()) & \
#            (data.edge_index[1].view(-1) <= node_indices.max())
