import argparse
from model import CausalGCN, CausalGIN, CausalGAT, GINNet, GCNNet, GATNet, CausalGCN_share, CausalGIN_share, \
    CausalGAT_share, CausalGCN_begin
import numpy as np
import random
import torch
...
def parse_args():
    ...
    parser.add_argument('--data_num', type=int, default=140000)
    parser.add_argument('--node_num', type=int, default=85)
...
def get_model(args):
    ...
    def model_func7(num_features, num_classes):
        return CausalGCN_share(num_features, num_classes, args)

    def model_func8(num_features, num_classes):
        return CausalGIN_share(num_features, num_classes, args)

    def model_func9(num_features, num_classes):
        return CausalGAT_share(num_features, num_classes, args)

    def model_func10(num_features, num_classes):
        return CausalGCN_begin(num_features, num_classes, args)
    ...
    elif args.model == "CausalGCN_share":
        model_func = model_func7
    elif args.model == "CausalGIN_share":
        model_func = model_func8
    elif args.model == "CausalGAT_share":
        model_func = model_func9
    elif args.model == "CausalGCN_begin":
        model_func = model_func10
    else:
        assert False
    return model_func
